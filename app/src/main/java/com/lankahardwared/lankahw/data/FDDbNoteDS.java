...
public class FDDbNoteDS {
    ...
    public double getDebtorBalance(String DebCode) {
        ...
        return totbal;
    //    return totbal - totbal1; commented by rashmi 2019-05-27
    ...
    public int DayDifference(String sDate) {
        ...
        try {
            ...
            Calendar c2 = Calendar.getInstance();
            Date Date2 = sdf.parse(sDate);
            String outputDate = sdf1.format(Date2);
            Date Date3 = sdf1.parse(outputDate);
            c2.setTime(Date3);
    ...
    public int getOldestFDDBNoteDate(String debCode)
    {
        if (dB == null) {
            open();
        } else if (!dB.isOpen()) {
        ...
        try {
            ...
            String selectQuery = "SELECT txndate FROM " + DatabaseHelper.TABLE_FDDBNOTE + " WHERE DebCode ='" + debCode + "' and totbal > '0' ORDER BY txndate ASC LIMIT 1";
            cursor = dB.rawQuery(selectQuery, null);

            while (cursor.moveToNext())
            {
                noOfDays = DayDifference(cursor.getString(cursor.getColumnIndex(DatabaseHelper.FDDBNOTE_TXN_DATE)));
            }
        } catch (Exception e) {
            Log.v(TAG, e.toString());
    ...
    public ArrayList<FDDbNote> getAllRecords(String debcode, boolean isSummery) {
        ...
        try {
            ...
            if (isSummery)
                selectQuery = "select * from " + DatabaseHelper.TABLE_FDDBNOTE + " WHERE " + " debcode='" + debcode + "' AND EnterAmt<>'' AND TotBal > 0.0 Order By " + DatabaseHelper.FDDBNOTE_TXN_DATE;
            else
                selectQuery = "select * from " + DatabaseHelper.TABLE_FDDBNOTE + " WHERE " + " debcode='" + debcode + "' AND TotBal > 0.0 Order By " + DatabaseHelper.FDDBNOTE_TXN_DATE;
...
//                values.put(DatabaseHelper.FDDBNOTE_TOT_BAL, Double.parseDouble(fddb.getFDDBNOTE_TOT_BAL()) - Double.parseDouble(fddb.getFDDBNOTE_ENTER_AMT()));
                values.put(DatabaseHelper.FDDBNOTE_TOT_BAL, fddb.getFDDBNOTE_TOT_BAL());
//                values.put(DatabaseHelper.FDDBNOTE_ENTER_AMT, "");
//                values.put(DatabaseHelper.FDDBNOTE_REMARKS, "");
                dB.update(DatabaseHelper.TABLE_FDDBNOTE, values, DatabaseHelper.FDDBNOTE_REFNO + "=?", new String[] { fddb.getFDDBNOTE_REFNO()});
            }
        } catch (Exception e) {
            Log.v(TAG + " Exception", e.toString());
        } finally {
    ...
    public void UpdateFddbNoteBalanceForReceipt(ArrayList<RecDet> list) {
        ...
        try {
            for (RecDet recDet : list) {
                ...
                values.put(DatabaseHelper.FDDBNOTE_ENTER_AMT, "");
                dB.update(DatabaseHelper.TABLE_FDDBNOTE, values, DatabaseHelper.FDDBNOTE_REFNO + "=?", new String[] { recDet.getFPRECDET_REFNO1() });
            }
        } catch (Exception e) {
            Log.v(TAG + " Exception", e.toString());
        } finally {
    ...
    public FDDbNote getDetailsOfSelectedRefNo(String refNo) {
        if (dB == null) {
            open();
        } else if (!dB.isOpen()) {
            open();
        ...
        try {
            String selectQuery;
            selectQuery = "select * from " + DatabaseHelper.TABLE_FDDBNOTE + " WHERE " + " debcode='" + refNo + "'";
    ...
    public boolean isAnyActive() {
        ...
        Cursor cursor = null;
        try {
            String selectQuery = "SELECT * FROM " + DatabaseHelper.TABLE_FDDBNOTE + " WHERE " + DatabaseHelper.FINVRHED_IS_ACTIVE + "='1'";
            cursor = dB.rawQuery(selectQuery, null);
