        ...
        try {
            ...
            for (FreeHed freehed : list) {
                ...
                if (cursor_ini.getCount() > 0) {
                    String selectQuery = "SELECT * FROM " + dbHelper.TABLE_FFREEHED + " WHERE " + dbHelper.FFREEHED_REFNO + "='" + freehed.getFFREEHED_REFNO() + "'";
                    cursor = dB.rawQuery(selectQuery, null);

                    if (cursor.getCount() > 0) {
                        returnID = (int) dB.update(dbHelper.TABLE_FFREEHED, values, dbHelper.FFREEHED_REFNO + "='" + freehed.getFFREEHED_REFNO() + "'", null);
                    } else {
