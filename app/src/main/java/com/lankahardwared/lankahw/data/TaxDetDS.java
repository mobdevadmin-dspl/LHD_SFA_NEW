...
public class TaxDetDS {
    ...
    public String calculateReverseTax(String itemCode, BigDecimal amt) {
    ...
    public String calculateReverseTaxFromDebTax(String debtorCode, String itemCode, BigDecimal amt) {
        ...
        String comCode = new ItemsDS(context).getTaxComCodeByItemCodeBeforeDebTax(itemCode, debtorCode);
        ArrayList<TaxDet> list = new TaxDetDS(context).getTaxInfoByComCode(comCode);
        BigDecimal tax = new BigDecimal("0");
    ...
    public int deleteAllTaxDets() {
        ...
        try {
            ...
            cursor = dB.rawQuery("SELECT * FROM " + dbeHelper.TABLE_FTAXDET, null);
            count = cursor.getCount();
            if (count > 0) {
                success = dB.delete(dbeHelper.TABLE_FTAXDET, null, null);
