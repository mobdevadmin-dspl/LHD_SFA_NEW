...
public class ReceiptAdapter1 extends ArrayAdapter<FDDbNote> {
    ...
    {
        paidList = new PaymentAllocateDS(context).getAllPaidRecords(refNo, comRefNo);
    ...
    public void EditPaymentDialogBox(final ViewHolder viewHolder, final FDDbNote fdDbNote, final String payComRefNo)
    {
        ...
        updatedOrderList = new PaymentAllocateDS(context).getAllPaidRecordsByTwoRefNo(fdDbNote.getFDDBNOTE_REFNO(), payComRefNo);
        ...
        alertDialogBuilder.setCancelable(false).setNegativeButton("DONE", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                ...
                if (Double.parseDouble(getTotalPaidAmt(fdDbNote.getFDDBNOTE_REFNO()))>0)
                {
                    ...
                    updatedOrderList1 = new PaymentAllocateDS(context).getAllPaidRecordsByTwoRefNo(fdDbNote.getFDDBNOTE_REFNO(), payComRefNo);
                    ...
                    {
                        new PaymentAllocateDS(context).createOrUpdatePaymentAllocate(paymentAllocateArrayList);
                    }

                    new PaymentAllocateDS(context).updatePaidAmount(fdDbNote.getFDDBNOTE_REFNO(), getTotalPaidAmt(fdDbNote.getFDDBNOTE_REFNO()));
                }
                else
                {
                    viewHolder.lblDueAmt.setText(String.format("%,.2f", Double.parseDouble(fdDbNote.getFDDBNOTE_TOT_BAL())));
